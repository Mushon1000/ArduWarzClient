...
public:
	...
	static const int EVQUEUE_SIZE = 5;
	...
	boolean enqueueEvent(int ev_code, int ev_param, int ev_param2);

	// tries to extract an event from the queue;
	// returns true if successful, false if the
	// queue is empty (the parameteres are not touched
	// in this case)
	boolean dequeueEvent(int* ev_code, int* ev_param, int* ev_param2);
